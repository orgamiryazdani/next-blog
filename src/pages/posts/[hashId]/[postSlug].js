    ...
    return (
        <Layout>
            <div className="md:max-w-screen-lg container mx-auto">
                <header className="flex flex-col max-w-screen-md md:flex-row gap-y-5 md:justify-between md:items-start mb-12 mx-auto">
                    {/* author data */}
                    <div className="flex items-stretch">
                        <img src='/images/saheb.jpg' className="w-14 h-14 md:w-20 md:h-20 rounded-full ring-2 ring-white" alt={post.author.name} />
                        <div className="flex flex-col mr-4 justify-between">
                            <div>
                                <Link href={`/blogs/${post.category.englishTitle}`} legacyBehavior>
                                    <a className="bg-white border border-blue-500 text-xs text-blue-500 px-3 py-1 mr-2 rounded-full transition-all duration-300 hover:bg-blue-500 hover:text-white  ">
                                        {post.category.title}
                                    </a>
                                </Link>
                                <span className="font-extrabold text-base">{post.author.name}</span>
                            </div>
                    ...
                    <div className="flex">
                        ...
                        <button className="mr-4 border border-gray-300 text-gray-500 hover:text-gray-600 rounded-full px-3 py-1 flex items-center">
                            <span className="ml-1 text-xs">
                                {post.isBookmarked ? 'ذخیره' : 'ذخیره شد'}
                            </span>
                            {post.isBookmarked ? <SolideBookmarkIcon className="w-6 h-6 fill-current" /> : <BookmarkIcon className="w-6 h-6 stroke-current" />}
                        </button>
                    </div>
                </header>
                <main className="prose mb-8 prose-xl max-w-screen-md mx-auto prose-img:rounded-xl prose-a:text-blue-500 prose-slate prose-h1:text-3xl prose-h1:font-black prose-h2:text-2xl prose-h2:font-extrabold prose-p:text-base prose-p:leading-8 md:prose-p:text-lg md:prose-p:leading-10">
                    <h1>{post.title}</h1>
...
`}</pre>
                ...
                <section>
                    <ul className="flex items-center flex-wrap gap-x-4 mb-6">
                        {["فرانت اند", "جاوا اسکریپت", "ریکت", "Next.js",].map((tag, index) => {
                            return (
                                <li key={index} className="px-3 py-1 rounded-2xl bg-gray-200 hover:bg-gray-100 transition-all cursor-pointer text-gray-600 text-sm mb-3 block duration-300">{tag}</li>
                            )
                        })}
                    </ul>
